...
SPLIT_DATASET_RATIO=${SPLIT_DATASET_RATIO:-0.005}
...
SWANLAB_PROJECT=${SWANLAB_PROJECT:-"data-synthesis-w-kg-v3"}
...
# 分布式训练命令
NPROC_PER_NODE=$NPROC_PER_NODE swift rlhf \
    --rlhf_type dpo \
    --model "$MODEL_PATH" \
    --dataset $DATASET \
    ...
    --num_train_epochs "$NUMBER_TRAIN_EPOCHS" \
