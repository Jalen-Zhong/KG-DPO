...
from kg_dpo.models import Chunk, OpenAIModel, Tokenizer
from kg_dpo.models.storage.base_storage import BaseGraphStorage
from kg_dpo.templates import KG_EXTRACTION_PROMPT
from kg_dpo.utils import (logger, pack_history_conversations, split_string_by_multi_markers,
                            handle_single_entity_extraction, handle_single_relationship_extraction,
                            detect_if_chinese)
from kg_dpo.operators.merge_kg import merge_nodes, merge_edges
...
async def extract_kg(
    ...
    async for result in tqdm_async(
        ...
        total=len(chunks),
        desc="[1/3]Extracting entities and relationships from chunks",
        unit="chunk",
    ):
        try:
            if progress_bar is not None:
                progress_bar(len(results) / chunk_number, desc="[1/3]Extracting entities and relationships from chunks")
            results.append(await result)
            if progress_bar is not None and len(results) == chunk_number:
                progress_bar(1, desc="[1/3]Extracting entities and relationships from chunks")
